...
describe("Configuration loader", function()
  ...
  describe("validations", function()
    ...
    it("enforces listen addresses format", function()
      ...
      assert.is_nil(conf)
      assert.equal("admin_listen must be of form: [off] | <ip>:<port> [ssl] [http2], [... next entry ...]", err)

      conf, err = conf_loader(nil, {
        proxy_listen = "127.0.0.1"
      })
      assert.is_nil(conf)
      assert.equal("proxy_listen must be of form: [off] | <ip>:<port> [ssl] [http2], [... next entry ...]", err)
    end)
